...
int AK15NanoFlatTreeProducer(const char* inputFile = "codex_max5_v2_numEvent5_Nano_numEvent5.root",
  ...
  while (reader.Next()) {
    ...
    for (UInt_t j = 0; j < *nJet; ++j) {
      if (bestJetIdx < 0 || jet_pt[j] > jet_pt[bestJetIdx]) {
        bestJetIdx = static_cast<Int_t>(j);
      }
    }

    if (bestJetIdx < 0 ||
        jet_pt[bestJetIdx] <= 170.0f ||
        std::abs(jet_eta[bestJetIdx]) >= 3.0f) {
      ++noPassingJet;
      continue;
    }
    ...
    for (UInt_t i = 0; i < *nMuon; ++i) {
      ...
      const double dphi = std::abs(deltaPhi(jet_phi[bestJetIdx], muon_phi[i]));
      if (muon_pt[i] > 30.0f && std::abs(muon_eta[i]) < 2.5f && iso < 0.3 && dphi > 1.5) {
        bestLeptonPdgId = 13;
        bestLeptonIdx = static_cast<Int_t>(i);
        bestLeptonPt = muon_pt[i];
        bestLeptonEta = muon_eta[i];
        bestLeptonPhi = muon_phi[i];
        bestLeptonIso = iso;
        bestLeptonDeltaPhi = dphi;
        break;
      }
    }
    ...
    if (bestLeptonIdx < 0) {
      for (UInt_t i = 0; i < *nElectron; ++i) {
        ...
        const double dphi = std::abs(deltaPhi(jet_phi[bestJetIdx], electron_phi[i]));
        if (electron_pt[i] > 30.0f && std::abs(electron_eta[i]) < 2.5f && iso < 0.3 && dphi > 1.5) {
          bestLeptonPdgId = 11;
          bestLeptonIdx = static_cast<Int_t>(i);
          bestLeptonPt = electron_pt[i];
          bestLeptonEta = electron_eta[i];
          bestLeptonPhi = electron_phi[i];
          bestLeptonIso = iso;
          bestLeptonDeltaPhi = dphi;
          break;
        }
    ...
    selectedSourceJetIdx =
        jet_sourceIdx ? (*jet_sourceIdx)[bestJetIdx] : bestJetIdx;
    nInputAK15 = jet_originalMultiplicity
                     ? (*jet_originalMultiplicity)[bestJetIdx]
