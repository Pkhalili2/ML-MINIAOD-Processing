...
def emit(shell_name, key, default):
    ...
    if key in ("lepton_mode", "muon_iso_branch", "lumi_mask"):
...
root -l -b -q "PhysicsAnalysisTreeProducer.C+(\"${ROOT_INPUT}\",\"${ROOT_OUTPUT}\",\"${ROOT_SAMPLE}\",${IS_DATA},${MAX_EVENTS},${JET_PT_MIN},${JET_ETA_MAX},\"${ROOT_LEPTON_MODE}\",${MUON_PT_MIN},${MUON_ETA_MAX},${MUON_ISO_MAX},${MIN_DPHI},\"${ROOT_ISO_BRANCH}\",\"${ROOT_LUMI_MASK}\")"
