...
Options:
  ...
  --config-type mc|data          NanoAOD config for phase1. Default: mc
  ...
  --request-disk SIZE            HTCondor disk request, use numeric KB. Default: 12582912
  ...
  --ak15-leading-only 0|1        Store custom AK15 tables only for the highest-pt jet
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --mode) MODE="$2"; shift 2 ;;
    --config-type) CONFIG_TYPE="$2"; shift 2 ;;
    ...
    --ak15-leading-only) AK15_LEADING_ONLY="$2"; shift 2 ;;
...
make_args=(
  ...
  --save-nano "${SAVE_NANO}"
  --ak15-leading-only "${AK15_LEADING_ONLY}"
...
echo "NanoAOD config type: ${CONFIG_TYPE}"
