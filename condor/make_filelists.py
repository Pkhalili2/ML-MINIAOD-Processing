...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=("mc", "data"),
        default="mc",
        help="NanoAOD config to run in phase1: mc for simulated signal/MC, data for data-style inputs.",
    )
    ...
    parser.add_argument(
        ...
        choices=("0", "1"),
        default="0",
        help="Record whether Stage 1 stores only the highest-pt custom AK15 jet.",
    )
...
def discover_inputs(spec):
    ...
    if path.exists():
        if path.is_dir():
            return sorted(
                str(item) for item in path.rglob("*.root") if item.is_file() and item.stat().st_size > 0
            )
    ...
    if has_glob(spec):
        return sorted(
            item
            for item in glob.glob(spec, recursive=True)
...
def main():
    ...
    manifest = {
        ...
        "ak15_leading_only": args.ak15_leading_only,
